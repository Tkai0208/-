...
class ClusterContrastTrainer_PCLMP(object):
    ...
    def train(self, epoch, data_loader_ir, data_loader_rgb, optimizer, 
        print_freq=10, train_iters=400, i2r=None, r2i=None, has_cross_loss=True):
        ...
        for i in range(train_iters):
            ...
            cross_loss, unified_loss, online_loss, align_loss = torch.tensor(0.), torch.tensor(0.).cuda(), torch.tensor(0.).cuda(), torch.tensor(0.).cuda()

            # cross contrastive learning
            if r2i is not None:
                rgb2ir_labels = r2i[labels_rgb]
                ir2rgb_labels = i2r[labels_ir]
            ...
            if has_cross_loss:
                if alternate:
                    # accl
                    if epoch % 2 == 1:
                        cross_loss = 1 * self.memory_rgb(f_out_ir, ir2rgb_labels.long())  #, detach=cross_detach)
                    else:
                        cross_loss = 1 * self.memory_ir(f_out_rgb, rgb2ir_labels.long())  #, detach=cross_detach)
                else:
            ...
            if (i + 1) % print_freq == 0:
                print('Epoch: [{}][{}/{}]\t'
                      ...
                      .format(epoch, i + 1, len(data_loader_rgb),
                              batch_time.val, batch_time.avg,
                              data_time.val, data_time.avg,
                              losses.val, losses.avg, loss_ir, loss_rgb, cross_loss))
