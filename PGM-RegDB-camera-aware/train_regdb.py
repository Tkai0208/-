...
def main():
    ...
    # 如果 Stage 1 的模型已存在，直接跳过 Stage 1
    stage1_model_path = osp.join('./logs/'+log_s1_name+'/'+str(args.trial), save_name+'model_best.pth.tar')
...
def main_worker_stage1(args, log_s1_name, save_name=''):
    ...
    sys.stdout = Logger(osp.join(args.logs_dir, str(trial)+save_name+'log.txt'))
    print("==========\nArgs:{}\n==========".format(args))
...
def main_worker_stage2(args, log_s1_name, log_s2_name, save_name=''):
    ...
    # Create model
    model = create_model(args)
    checkpoint = load_checkpoint(osp.join('./logs/'+log_s1_name+'/'+str(trial), save_name+'model_best.pth.tar'))
    ...
    for epoch in range(args.epochs):
        ...
        # [Camera-aware] 提取每个训练样本的相机标签，用于后续按相机分组计算聚类中心
        ...
        def generate_cluster_features(labels, features, cids):
            ...
            这样生成的 camera-aware cluster feature 将直接用于 cross-modality Hungarian matching。
            """
        ...
        i2r, r2i, proxy_pseudo_labels = None, None, None
        has_cross_loss = True
...
##############################
            ...
            # for trial in range(1):
            ngall = len(gall_label)
            gall_feat_fc = extract_gall_feat(model_eval, gall_loader,ngall)
            # fc feature
            distmat = np.matmul(query_feat_fc, np.transpose(gall_feat_fc))
            cmc, mAP, mINP = eval_regdb(-distmat, query_label, gall_label)
