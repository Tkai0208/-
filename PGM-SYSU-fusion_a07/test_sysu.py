...
def extract_gall_feat(model,gall_loader,ngall,feat_concate=False):
    pool_dim = 1024 if feat_concate else 2048
    net = model
    net.eval()
...
def extract_query_feat(model,query_loader,nquery,feat_concate=False):
    pool_dim = 1024 if feat_concate else 2048
    net = model
    net.eval()
...
def main_worker(args):
    ...
    feat_concate = False
    log_name='sysu_s2_fusion_a07'  # model path (Stage 2 final model, alpha=0.7)
...
#################################
    ...
    checkpoint = load_checkpoint(osp.join(args.logs_dir, 'train_model_best.pth.tar'))
    ...
    query_feat_fc = extract_query_feat(model,query_loader,nquery,feat_concate=feat_concate)
    for trial in range(10):
        ...
        gall_feat_fc = extract_gall_feat(model,trial_gall_loader,ngall,feat_concate=feat_concate)
        # fc feature
