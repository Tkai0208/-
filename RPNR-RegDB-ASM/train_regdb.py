...
from collections import OrderedDict  # IMPROVEMENT: for gray feature extraction
...
def compute_similarity_matrix(features_a, features_b):
    """
    计算余弦相似度矩阵
    输入: features_a [N_a, D], features_b [N_b, D]
    输出: similarity_matrix [N_a, N_b]
    """
    # 确保在同一设备上
    if features_a.is_cuda and not features_b.is_cuda:
        features_b = features_b.cuda()
    elif features_b.is_cuda and not features_a.is_cuda:
...
def augmented_matching_fusion(cluster_features_visible, cluster_features_augmented, 
    ...
    """
    Augmented Matching: 使用增强模态辅助跨模态匹配
    论文公式(8): 融合Visible-to-IR和Augmented-to-IR相似度矩阵
    
    输入:
        cluster_features_visible: [N_v, D] 可见光聚类中心
        cluster_features_augmented: [N_a, D] 增强模态聚类中心 (N_a = N_v)
        cluster_features_ir: [N_r, D] 红外聚类中心
        gamma_v: Visible矩阵的收缩因子
        gamma_a: Augmented矩阵的收缩因子
    
    输出:
        M_var: [N_v, N_r] 融合后的相似度矩阵
    """
    # 确保所有输入都在同一设备上
    device = cluster_features_visible.device
    cluster_features_augmented = cluster_features_augmented.to(device)
    cluster_features_ir = cluster_features_ir.to(device)
    
    # 计算M_vr: Visible-to-Infrared相似度矩阵 [N_v, N_r]
    M_vr = compute_similarity_matrix(cluster_features_visible, cluster_features_ir)
    
    # 计算M_ar: Augmented-to-Infrared相似度矩阵 [N_a, N_r]
    M_ar = compute_similarity_matrix(cluster_features_augmented, cluster_features_ir)
    
    # 公式(8): 融合两个矩阵
    # m_var_ij = 1 / [(1+exp(-gamma_v*m_vr_ij)) * (1+exp(-gamma_a*m_ar_ij))]
    sigmoid_vr = torch.sigmoid(gamma_v * M_vr)
    sigmoid_ar = torch.sigmoid(gamma_a * M_ar)
    
    M_var = 1.0 / ((1.0 + torch.exp(-gamma_v * M_vr)) * (1.0 + torch.exp(-gamma_a * M_ar)))
    
    print(f"Augmented Matching:")
    print(f"  Visible clusters: {len(cluster_features_visible)}")
    print(f"  Augmented clusters: {len(cluster_features_augmented)}")
    print(f"  IR clusters: {len(cluster_features_ir)}")
    print(f"  M_vr shape: {M_vr.shape}, M_ar shape: {M_ar.shape}")
    print(f"  M_var (fused) shape: {M_var.shape}")
    print(f"  gamma_v={gamma_v}, gamma_a={gamma_a}")
    
    return M_var, M_vr, M_ar

def extract_features_augmented(model, data_loader, print_freq=50, modal=1):
    """
    提取增强模态(Augmented)的特征
    通过对Visible图像应用Channel Augmentation生成增强图像
    modal=1: 使用visible_module提取特征
    """
    model.eval()
    ...
    with torch.no_grad():
        for i, (imgs, fnames, pids, _, _) in enumerate(data_loader):
            ...
            # 转换为灰度图: 使用标准ITU-R BT.601系数 (与ChannelAug.py的Gray类一致)
            gray = 0.2989 * imgs_gray[:, 0:1, :, :] + 0.5870 * imgs_gray[:, 1:2, :, :] + 0.1140 * imgs_gray[:, 2:3, :, :]
            imgs_gray = gray.repeat(1, 3, 1, 1)  # [B, 3, H, W]
            
            imgs_gray = imgs_gray.cuda()
            
            # 提取特征（使用与RGB相同的encoder但输入是灰度图）
            # 模型forward参数: forward(x1, x2, modal=0, label_1=None, label_2=None)
            # modal=1 表示使用visible_module
            outputs = model(imgs_gray, None, modal=modal)
            
            for fname, output, pid in zip(fnames, outputs, pids):
                features[fname] = output.squeeze()
                labels[fname] = pid
                
            if (i + 1) % print_freq == 0:
                print(f'Extract augmented features: [{i+1}/{len(data_loader)}]')
...
def main():
    ...
    log_s1_name = 'regdb_gamma_s1'
    log_s2_name = 'regdb_gamma_s2'
    
    # 检查 Stage 1 是否已完成
    trial = args.trial
    stage1_model_path = osp.join('./logs/'+log_s1_name+'/'+str(trial), 'model_best.pth.tar')
    if osp.exists(stage1_model_path):
        print(f"Stage 1 model found: {stage1_model_path}")
        print("Skipping Stage 1, running Stage 2 directly...")
        main_worker_stage2(args,log_s1_name,log_s2_name)
    else:
        print("Stage 1 model not found, running full training...")
...
def main_worker_stage2(args,log_s1_name,log_s2_name):
    ...
    for epoch in range(args.epochs):
        ...
        # 更新聚类数量（可能因标签校正而改变）
        num_cluster_ir = len(cluster_features_ir)
        num_cluster_rgb = len(cluster_features_rgb)
        ...
        # 提取Augmented模态特征 (通过对Visible图像做Channel Augmentation)
        cluster_loader_rgb_aug = get_test_loader(dataset_rgb, args.height, args.width,
                                             args.batch_size, args.workers, 
                                             testset=sorted(dataset_rgb.train))
        
        # 使用augmented模式提取特征 (将RGB转为灰度)
        features_augmented, _ = extract_features_augmented(model, cluster_loader_rgb_aug, 
                                                           print_freq=50, modal=1)
        del cluster_loader_rgb_aug
        features_augmented = torch.cat([features_augmented[f].unsqueeze(0) for f, _, _ in sorted(dataset_rgb.train)], 0)
        
        # 使用与RGB相同的伪标签计算Augmented聚类中心
        # 论文说明: augmented clusters与visible clusters共享相同伪标签
        cluster_features_augmented = generate_cluster_features(pseudo_labels_rgb, features_augmented)
        print(f"Augmented clusters: {len(cluster_features_augmented)}")
        
        # Augmented Matching融合 (公式8)
        print("Augmented Matching: Fusing Visible and Augmented similarities...")
        gamma_v = 2.0  # Visible矩阵收缩因子 - 论文默认
        gamma_a = 1.0  # Augmented矩阵收缩因子 - 论文默认
        
        M_var, M_vr, M_ar = augmented_matching_fusion(
            cluster_features_rgb, 
            cluster_features_augmented,
            cluster_features_ir,
            gamma_v=gamma_v,
            gamma_a=gamma_a
        )
        ...
        # 使用融合后的矩阵M_var作为匹配矩阵
        # M_var shape: [N_visible, N_ir]
        matching_matrix = M_var  # [N_visible, N_ir]
        
        # 使用Optimal Transport (Sinkhorn)求解跨模态匹配，替代直接argmax
        # M_var是相似度矩阵，需取倒数变为距离矩阵(cost)，与RPNR原代码保持一致
        ...
        rgb_to_ir_hard = np.argmax(result, axis=1)  # [N_visible]
        ir_to_rgb_hard = np.argmax(result, axis=0)  # [N_ir]
        
        # i2r: IR簇ID -> RGB簇ID (需要N_ir个映射)
        i2r = {i: int(ir_to_rgb_hard[i]) for i in range(len(ir_to_rgb_hard))}
        # r2i: RGB簇ID -> IR簇ID (需要N_visible个映射)
        r2i = {i: int(rgb_to_ir_hard[i]) for i in range(len(rgb_to_ir_hard))}
        
        print("Augmented Matching Done")

        # create hybrid memory with augmented matching
        cluster_features_hybrid = torch.zeros(cluster_features_ir.shape[0], cluster_features_ir.shape[1]).cuda()
        cluster_features_ir = cluster_features_ir.cuda()
        cluster_features_rgb = cluster_features_rgb.cuda()
        
        for j in range(num_cluster_ir):
            # 对所有RGB簇按匹配概率加权，融合到IR簇j
            # matching_matrix shape: [N_visible, N_ir], 第j列是所有Visible簇对IR簇j的权重
        ...
        # 训练（无灰度加载器）
        trainer.train(epoch, train_loader_ir, train_loader_rgb, 
                      data_loader_gray=None,  # 无灰度
                      optimizer=optimizer, 
