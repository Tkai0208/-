...
class llcm_rgb(BaseImageDataset):
    """
    sysu_rgb
    ...
    def __init__(self, root, verbose=True, **kwargs):
        super(llcm_rgb, self).__init__()
        self.dataset_dir = root  # osp.join(root, self.dataset_dir)
        self.train_list = osp.join(root, 'idx', 'train_vis.txt')    # for training
        self.test_list = osp.join(root, 'idx', 'test_vis.txt')  # not use

        self._check_before_run()
        train = self._process_dir(self.train_list)
        test = self._process_dir(self.test_list)

        print("=> llcm_rgb loaded")
        self.train = train
        self.test = test
