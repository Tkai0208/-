...
def get_single_model(model):
    """Bypass DataParallel for eval/clustering to avoid 300s→2s slowdown."""
    if isinstance(model, nn.DataParallel):
        return model.module
    return model

# ========== Augmented Matching: 增强匹配函数 (来自RegDB) ==========
...
def augmented_matching_fusion(cluster_features_visible, cluster_features_augmented, 
    ...
    print(f"Augmented Matching Fusion: gamma_v={gamma_v}, gamma_a={gamma_a}")
    print(f"  M_vr range: [{M_vr.min():.4f}, {M_vr.max():.4f}]")
    print(f"  M_ar range: [{M_ar.min():.4f}, {M_ar.max():.4f}]")
...
def main():
    ...
    log_base_name = 'baseline/sysu_s1'
    log_s2_name = 'sysu_asm_s2'  # ASM Stage 2
    main_worker(args, log_base_name, log_s2_name)

def main_worker(args, log_base_name, log_s2_name):
    start_epoch=0
    best_mAP=0
    best_R1=0
    best_epoch=0
    ...
    checkpoint = load_checkpoint(osp.join(log_base_name, 'model_best.pth.tar'))
    ...
    for epoch in range(args.epochs):
        with torch.no_grad():
            ...
            # ========== Augmented Matching: 提取灰度增强特征 ==========
            print('==> Extract augmented (grayscale) features for RGB data')
            cluster_loader_rgb = get_test_loader(dataset_rgb, args.height, args.width,
                                             256, args.workers, 
                                             testset=sorted(dataset_rgb.train))
            features_rgb_aug = extract_features_augmented(get_single_model(model), cluster_loader_rgb, print_freq=50, modal=1)
            del cluster_loader_rgb
            features_rgb_aug = torch.cat([features_rgb_aug[f].unsqueeze(0) for f, _, _ in sorted(dataset_rgb.train)], 0)
            print(f"Augmented features shape: {features_rgb_aug.shape}")
        ...
        def generate_cluster_features_corr(labels, features):
            ...
            for i, feat in centers.items():
                ...
                # 使用PyTorch的norm，避免CUDA tensor转numpy的问题
                feat_cpu = feat.cpu()
                normalized_feat = feat_cpu / np.linalg.norm(feat_cpu, axis=1, keepdims=True)
        ...
        # ========== Augmented Matching: 生成增强特征的cluster features ==========
        cluster_features_rgb_aug, _ = generate_cluster_features_corr(pseudo_labels_rgb, features_rgb_aug)
        print(f"Cluster features augmented shape: {cluster_features_rgb_aug.shape}")
        ...
        ######################## Augmented Matching Fusion
        print("Augmented Matching: RGB + Augmented <-> IR")
        
        # 使用增强匹配融合（替代原来的soft_matching_ot）
        # gamma_v=2.0, gamma_a=1.0 (比例 2:1) 用于 SYSU-MM01
        gamma_v = 2.0
        gamma_a = 1.0
        M_var, M_vr, M_ar = augmented_matching_fusion(
            cluster_features_rgb, cluster_features_rgb_aug, cluster_features_ir,
        ...
        # 使用Optimal Transport (Sinkhorn)求解跨模态匹配，与RPNR原代码及RegDB ASM保持一致
        ...
        trainer.train(epoch, train_loader_ir, train_loader_rgb, 
                      data_loader_gray=None, optimizer=optimizer,
